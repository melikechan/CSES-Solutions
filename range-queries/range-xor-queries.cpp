...
/*
    ...
    I have changed the way of building segment tree by taking the XOR.
*/
...
void build(vector<ll> &data, int idx, int tl, int tr)
{
    ...
    else
    {
        ...
        segtree[idx] = segtree[2 * idx] ^ segtree[2 * idx + 1]; // idx contains xor of left and right subtree (as we have divided into two subsegments)
    }
}
...
ll query(int idx, int l, int r, int tl, int tr)
{
    ...
    int tm = (tl + tr) / 2;
    return query(2 * idx, l, min(r, tm), tl, tm) ^ query(2 * idx + 1, max(tm + 1, l), r, tm + 1, tr); // answer of the query is xor of subsegments
}
